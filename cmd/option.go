...
func optionThreshold(threshold string, pid int32, nonInteractive bool) option {
	...
	fmt.Printf("  %s (now:%f): %s\n", color.White("load1", color.B), m["load1"], "Load avarage for 1 minute.")
	fmt.Printf("  %s (now:%f): %s\n", color.White("load5", color.B), m["load5"], "Load avarage for 5 minutes.")
	fmt.Printf("  %s (now:%f): %s\n", color.White("load15", color.B), m["load15"], "Load avarage for 15 minutes.")
	fmt.Printf("%s\n", color.White("Available Operators", color.B))
	fmt.Printf("  %s\n", "+, -, *, /, ==, !=, <, >, <=, >=, not, and, or, !, &&, ||")
	fmt.Println("")
